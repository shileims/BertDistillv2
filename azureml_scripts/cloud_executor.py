...
COMPUTE_TARGET_TO_VM_SIZE = { "NC24s-v3" : "STANDARD_NC24S_V3", "OXO-ML-CA-NC12": "STANDARD_NC12S_V2", "compute":"STANDARD_NC12S_V2","compute1":"STANDARD_NC6S_V2",
                              ...
                              "lei-compute": "STANDARD_NC24S_V2",
                              "lei4g1": "STANDARD_NC24S_V2",
                              "lei4g2": "STANDARD_NC24S_V2",
...
configs_sstock = {}
configs_sstock['config_dummtraining.json'] = ['lei-mmtraining', 'sstock']
configs_sstock['config_shilei.json'] = [['lei4g1', 'sstock'], ['lei4g2', 'sstock'], ['lei4g3', 'sstock']]
configs_sstock['config.json'] = ['lei-compute', 'lei2021datastore']
...
def register_store(workspace=None, datastore_name='tmp', account_name="zdaxseasia", container_name="v-miazhang", account_key="m0bb7W1egG4u/8mDk+3iIR9xIuZGXySqfApWwVP4tJ+YVXfrFbNbTyw1v8FZzG00LvXbhkljf+bTKx9xkMhveg=="):
    ...
    print("Registered blob datastore with name: %s" % "tmp")
...
# parser.add_argument('--workspace-config',       type=str,                    dest='workspace_config',       default='config_shilei.json',      help='config.json | config_hi.json for high-perf workspace')
...
parser.add_argument('--workspace-config',       type=str,                    dest='workspace_config',       default='config_dummtraining.json',      help='config.json | config_hi.json for high-perf workspace')
...
parser.add_argument('--datastore',              type=str,                    dest='datastore',              default='sstock', help='Name of the blob storage. You can work on your own azure storage account by using `register_azure_storage.py` to register it. Defaults to workspaceblobstore')
...
parser.add_argument('--config_type', type=str, default='configs_defaultsinglegpu', choices=['configs_sstock', 'configs_u2netdatastore', 'configs_defaultsinglegpu', 'configs_defaultcpus'])
...
parser.add_argument('--experiment-tag-name',    type=str, dest='experiment_tag_name', default='swin_mini4', help='add experiment tag name')
...
parser.add_argument(script_exp_arg_name,        type=str,                    dest='experiment_name',        default='distill_refactor_dist_2M',                            help='Name of the Azure experiment. This can be anything you want.')
    ...
    if args.use_docker:
        est = PyTorch(source_directory=get_root_path(),
                         ...
                         image_registry_details=make_container_registry(
                             address=None,
                             username='lshi22',
                             password='HDJSJYl1991515'),
                         custom_docker_image='lshi22/pytorch:1.6.0-cuda10.1-apex-deepspeed-3.10-bss-tensorboard',
                         user_managed=True,
                         inputs=datasets)
    else:
