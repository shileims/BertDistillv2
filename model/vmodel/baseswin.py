...
class BaseSwin(nn.Module):
    def __init__(self, args):
        ...
        if self.is_distill:
            ...
            logger.log(f'Distilling and Loading pretrain vision model from {args.vlmodel_pretrain}')
            state_dict = torch.load(args.vlmodel_pretrain, map_location='cpu')['vision']
            smart_partial_load_model_state_dict(self.backbone, {k.replace('backbone.', ''):v for k, v in state_dict.items() if 'backbone.' in k}, rename=False)
    ...
    def projector_forward(self, latents):
        latents = F.linear(input=latents.float(),
                            weight=self.projector.weight.float(),
                          )
        return latents
